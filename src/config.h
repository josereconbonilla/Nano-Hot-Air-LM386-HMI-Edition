...
class HOTGUN_CFG : public CONFIG {
  ...
  private:
    uint16_t t_tip[3];
    const uint16_t def_tip[3] = {999, 1062, 1023}; // Default values of internal sensor readings at reference temperatures
    const uint16_t min_temp = 50;
    const uint16_t max_temp = 1023;
    const uint16_t def_temp = 350; // Default preset temperature
    const uint16_t def_fan = 128; // Default preset fan speed 0 - max_fan_speed
    const uint8_t  def_br = 64; // Default display brightness
    const uint16_t ambient_temp = 34;
