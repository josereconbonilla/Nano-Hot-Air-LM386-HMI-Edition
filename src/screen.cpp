...
SCREEN* mainSCREEN::show(void) {
  ...
  uint16_t tempH = pCfg->tempHuman(temp);// The preset temperature in the human readable units
  if (pHG->isCold()) {
    if (used) {
      pD->msgCold();
    } else {
...
void workSCREEN::init(void) {
  uint16_t temp_set   = pHG->presetTemp();
  uint16_t tempH      = pCfg->tempHuman(temp_set); //tempInternal
...
SCREEN* calibSCREEN::show(void) {
  ...
  uint16_t tempH = pCfg->tempHuman(temp);//tempInternal
